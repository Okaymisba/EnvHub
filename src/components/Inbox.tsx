...
export const Inbox: React.FC<InboxProps> = ({
  ...
  const [loadingStates, setLoadingStates] = React.useState<Record<string, 'accepting' | 'rejecting' | null>>({});

  const handleAccept = async (notification: Notification) => {
    if (notification.data?.invitation_id) {
      setLoadingStates(prev => ({ ...prev, [notification.id]: 'accepting' }));
      try {
        await onAcceptInvitation(notification.data.invitation_id);
      } finally {
        setLoadingStates(prev => ({ ...prev, [notification.id]: null }));
      }
    }
  };
  ...
  const handleReject = async (notification: Notification) => {
    if (notification.data?.invitation_id) {
      setLoadingStates(prev => ({ ...prev, [notification.id]: 'rejecting' }));
      try {
                  ...
                  <div className="flex gap-2 mt-3 sm:mt-0">
                    {!notification.data.accepted && !notification.data.rejected ? (
                      <>
                        ...
                        >
                          {loadingStates[notification.id] === 'accepting' ? (
                        ...
                        >
                          {loadingStates[notification.id] === 'rejecting' ? (
                            <span className="flex items-center">
